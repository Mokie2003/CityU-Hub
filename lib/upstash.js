/**
 * Upstash Redis 的极简 REST 客户端（只用到 pipeline）。
 *
 * 不引 @upstash/redis 依赖，直接用 fetch 打 REST 接口——统计只需要几个命令，
 * 少一个依赖就少一处构建风险。
 *
 * 环境变量名同时兼容两种接入方式：
 *   Vercel 市场的 Upstash 集成 → KV_REST_API_URL / KV_REST_API_TOKEN
 *   自己在 Upstash 建库         → UPSTASH_REDIS_REST_URL / UPSTASH_REDIS_REST_TOKEN
 *
 * 没配置时一律返回 null，调用方自己降级——统计不可用不该让站点功能坏掉。
 */
...
/**
 * 执行一组 Redis 命令。commands 形如 [['PFADD','uv:total','x'], ['INCR','view:y']]。
 * 返回 [{ result }, ...]；未配置时返回 null。
 */
...
/** 结果数组里取第 index 项的 result，缺项或报错时返回 fallback */
